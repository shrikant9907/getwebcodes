 ...
 */
function mycourse_site_logo( $args = array(), $echo = true ) {
	...
	/**
	 * Filters the arguments for `mycourse_site_logo()`.
	 *
	 * @param array  $args     Parsed arguments.
	 * @param array  $defaults Function's default arguments.
	 */
	$args = apply_filters( 'mycourse_site_logo_args', $args, $defaults );
	 ...
	 */
	$html = apply_filters( 'mycourse_site_logo', $html, $args, $classname, $contents );
 ...
 */
function mycourse_site_description( $echo = true ) {
	 ...
	 */
	$html = apply_filters( 'mycourse_site_description', $html, $description, $wrapper );
 ...
 * @return bool
 */
function mycourse_is_comment_by_post_author( $comment = null ) {
 ...
 */
function mycourse_filter_comment_reply_link( $link ) {
...
add_filter( 'comment_reply_link', 'mycourse_filter_comment_reply_link' );
 ...
 */
function mycourse_the_post_meta( $post_id = null, $location = 'single-top' ) {

	echo mycourse_get_post_meta( $post_id, $location ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in mycourse_get_post_meta().
 ...
 */
function mycourse_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$text = sprintf(
		wp_kses(
			...
			__( 'Edit <span class="screen-reader-text">%s</span>', 'mycourse' ),
			array(
				'span' => array(
					'class' => array(),
				),
	...
	return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . mycourse_get_theme_svg( 'edit' ) . '</span><span class="meta-text"><a href="' . esc_url( $edit_url ) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';

}

add_filter( 'edit_post_link', 'mycourse_edit_post_link', 10, 3 );
 ...
 */
function mycourse_get_post_meta( $post_id = null, $location = 'single-top' ) {
	 ...
	 */
	$disallowed_post_types = apply_filters( 'mycourse_disallowed_post_types_for_meta_output', array( 'page' ) );
	...
	if ( 'single-top' === $location ) {
		...
		$post_meta = apply_filters(
			'mycourse_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		 ...
		 */
		$post_meta = apply_filters(
			'mycourse_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
	...
	if ( $post_meta && ! in_array( 'empty', $post_meta, true ) ) {
		...
		<div class="post-meta-wrapper<?php echo esc_attr( $post_meta_wrapper_classes ); ?>">
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				 ...
				 *
				 * @since My Course 1.0
				 * @since My Course 1.1 Added the `$post_meta` and `$location` parameters.
				 *
				 * @param int    $post_id   Post ID.
				 ...
				 */
				do_action( 'mycourse_start_of_post_meta_list', $post_id, $post_meta, $location );
				...
				if ( post_type_supports( get_post_type( $post_id ), 'author' ) && in_array( 'author', $post_meta, true ) ) {
					...
					<li class="post-author meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post author', 'mycourse' ); ?></span>
							<?php mycourse_the_theme_svg( 'user' ); ?>
						</span>
				...
				if ( in_array( 'post-date', $post_meta, true ) ) {
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post date', 'mycourse' ); ?></span>
							<?php mycourse_the_theme_svg( 'calendar' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'categories', $post_meta, true ) && has_category() ) {
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Categories', 'mycourse' ); ?></span>
							<?php mycourse_the_theme_svg( 'folder' ); ?>
						</span>
						<span class="meta-text">
							<?php _ex( 'In', 'A string that is output before one or more categories', 'mycourse' ); ?> <?php the_category( ', ' ); ?>
						</span>
					</li>
					<?php
				...
				if ( in_array( 'tags', $post_meta, true ) && has_tag() ) {
					...
					<li class="post-tags meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Tags', 'mycourse' ); ?></span>
							<?php mycourse_the_theme_svg( 'tag' ); ?>
						</span>
						<span class="meta-text">
							<?php the_tags( '', ', ', '' ); ?>
						</span>
				...
				if ( in_array( 'comments', $post_meta, true ) && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					...
					<li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php mycourse_the_theme_svg( 'comment' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'sticky', $post_meta, true ) && is_sticky() ) {
					...
					<li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php mycourse_the_theme_svg( 'bookmark' ); ?>
						</span>
						<span class="meta-text">
							<?php _e( 'Sticky post', 'mycourse' ); ?>
						</span>
					</li>
					<?php
				 ...
				 *                          Accepts 'single-top' or 'single-bottom'.
				 */
				do_action( 'mycourse_end_of_post_meta_list', $post_id, $post_meta, $location );
 ...
 */
function mycourse_filter_wp_list_pages_item_classes( $css_class, $page, $depth, $args, $current_page ) {
...
add_filter( 'page_css_class', 'mycourse_filter_wp_list_pages_item_classes', 10, 5 );
 ...
 */
function mycourse_add_sub_toggles_to_main_menu( $args, $item, $depth ) {
	...
	if ( isset( $args->show_toggles ) && $args->show_toggles ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$toggle_duration      = mycourse_toggle_duration();

			// Add the sub menu toggle.
			$args->after .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'mycourse' ) . '</span>' . mycourse_get_theme_svg( 'chevron-down' ) . '</button>';
...
add_filter( 'nav_menu_item_args', 'mycourse_add_sub_toggles_to_main_menu', 10, 3 );
 ...
 */
function mycourse_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		$svg = TwentyTwenty_SVG_Icons::get_social_link_svg( $item->url );
		if ( empty( $svg ) ) {
			$svg = mycourse_get_theme_svg( 'link' );
		}
...
add_filter( 'walker_nav_menu_start_el', 'mycourse_nav_menu_social_icons', 10, 4 );
...
add_action( 'wp_head', 'mycourse_no_js_class' );
 ...
 */
function mycourse_body_classes( $classes ) {
...
add_filter( 'body_class', 'mycourse_body_classes' );
 ...
 */
function mycourse_get_the_archive_title( $title ) {

	$regex = apply_filters(
		'mycourse_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'mycourse_get_the_archive_title' );
 ...
 */
function mycourse_toggle_duration() {
	/**
	 ...
	 */
	$duration = apply_filters( 'mycourse_toggle_duration', 250 );
 ...
 */
function mycourse_unique_id( $prefix = '' ) {
	static $id_counter = 0;
